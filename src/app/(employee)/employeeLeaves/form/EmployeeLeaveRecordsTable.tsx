...
type Props = {
  ...
  onRowClick: (record: EmployeeLeaveRecord) => void;
};
...
export default function EmployeeLeaveRecordsTable({
  records,
  selectedYear,
  onYearChange,
  onRowClick,
  ...
  const columnHelper = createColumnHelper<EmployeeLeaveRecord>();
    ...
    () => [
      columnHelper.accessor(
        (row) => {
          ...
          const employeeNo = formatEmployeeNoDisplay(row.employeeNo);
        ...
        <TableBody>
          {loading ? (
            ...
            table.getRowModel().rows.map((row) => {
              const isEditable = row.original.leaveStatus === "Pending";

              return (
                <TableRow
                  key={row.id}
                  onClick={() => {
                    if (isEditable) onRowClick(row.original);
                  }}
                  className={isEditable ? "cursor-pointer hover:bg-muted" : ""}
                >
