...
import { selectPositionSchema } from "@/zod-schemas/position";
import { z } from "zod";

type Position = z.infer<typeof selectPositionSchema>;

export default function PositionTable({
  positions,
  onRowSelect,
}: {
  positions?: Position[]; // ✅ make optional
  onRowSelect?: (position: Position) => void;
}) {
  const safePositions = Array.isArray(positions) ? positions : []; // ✅ fallback
      ...
      <div className="flex justify-between items-center gap-1 flex-wrap">
        <p className="whitespace-nowrap font-bold">
          ...
          {`[${safePositions.length} ${
            safePositions.length !== 1 ? "total results" : "result"
          }]`}
        </p>
