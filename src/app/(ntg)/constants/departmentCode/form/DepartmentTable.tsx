...
import { selectDepartmentSchema } from "@/zod-schemas/department";
import { z } from "zod";

type Department = z.infer<typeof selectDepartmentSchema>;
...
export default function DepartmentTable({
  ...
  onRowSelect?: (department: Department) => void;
}) {
  const safeDepartments = Array.isArray(departments) ? departments : []; // ✅ fallback
  ...
  const table = useReactTable({
    data: safeDepartments, // ✅ safe value
    columns,
    state: {
      sorting,
      pagination: {
      ...
      <div className="flex justify-between items-center gap-1 flex-wrap">
        <p className="whitespace-nowrap font-bold">
          ...
          {`[${safeDepartments.length} ${
            safeDepartments.length !== 1 ? "total results" : "result"
          }]`}
        </p>
