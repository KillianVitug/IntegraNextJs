...
import PhilhealthContributionRateForm from "./PhilhealthContributionRateForm";
import PhilhealthContributionRateTable from "./PhilhealthContributionRateTable";

export default function PhilhealthContributionRatePage() {
  const [versions, setVersions] = useState<SelectStatutoryRuleVersionSchemaType[]>([]);
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(null);
  const [rows, setRows] = useState<SelectPhilhealthContributionRateSchemaType[]>([]);
  const [selectedRow, setSelectedRow] =
    useState<SelectPhilhealthContributionRateSchemaType | null>(null);

  const loadVersions = useCallback(async () => {
    const response = await fetch(
      "/api/constants/statutoryRuleVersion?ruleType=PHILHEALTH"
    );
  ...
  const loadRows = useCallback(async (versionId: number | null) => {
    ...
    const response = await fetch(
      `/api/constants/philhealthContributionRate?versionId=${versionId}`
    );
    const data = await response.json();
    setRows(data);
  }, []);
  ...
  return (
    <div className="flex flex-col gap-4">
      <StatutoryVersionSelector
        label="PhilHealth Parent Version"
        versions={versions}
        selectedVersionId={selectedVersionId}
        onChange={setSelectedVersionId}
      />
      <PhilhealthContributionRateForm
        selectedVersionId={selectedVersionId}
        selectedRow={selectedRow}
        onResetSelection={() => setSelectedRow(null)}
        onRefresh={() => loadRows(selectedVersionId)}
      />
      <PhilhealthContributionRateTable
        rows={rows}
